...
export const addSubCategory = async (
  ...
  try {
    ...
    if (!subCategoryData) {
      res
        .status(401)
        .json({ status: "Error", message: "This does not have data" });
      return;
    }
    ...
    console.log(
      "🚀 ~ file: addscrapingproduct.ts:45 ~ addSubCategory ~ error:",
      error
    );
...
export const addDetailsDb = async () => {
  try {
    ...
    console.log(
      "🚀 ~ file: addscrapingproduct.ts:89 ~ addDetailsDb ~ error:",
      error
    );
  }
};
      ...
      while (true) {
        try {
          ...
          const response = await axios.get(baseUrl, {
            headers: {
              Cookie:
                "LastSite=gb-en-001; JHYSESSIONID=Y14-fe3a42fd-9068-40c6-9a33-f00f93d7b72b; ROUTE=.accstorefront-595b85f95c-5d28z",
              "Cache-Control": "no-cache",
              "User-Agent": "Your-User-Agent",
              Accept: "*/*",
              "Accept-Encoding": "gzip, deflate, br",
...
export const addAccessories = async (
  ...
  try {
    ...
    console.log(
      "🚀 ~ file: addscrapingproduct.ts:199 ~ addAccessories ~ error:",
      error
    );
